...
/**
 * Servlet implementation class UsersEditConfirm
 */
@WebServlet("/UsersEditConfirm")
@MultipartConfig(location="", maxFileSize=1024 * 1024 * 2)
public class UsersEditConfirm extends HttpServlet {
	private static final long serialVersionUID = 1L;

    public UsersEditConfirm() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (errorMsg.isEmpty()) {// バリデーションエラーメッセージが空なら確認画面へ
			...
			//確認画面にフォワード
			request.getRequestDispatcher("/WEB-INF/jsp/usersEditConfirm.jsp").forward(request, response);
			...
			response.sendRedirect("/MyWebSite/UsersEdit");
		}
	}
